...
<?php 
if($_SESSION['LOGINAKS_VISITOR'] == "administrator")
{
?>
    <div class="row">
        <div class="col-lg-12" align="left">
            <button data-toggle="modal" data-target="#modalTambah" class="btn btn-danger btn-outline mb5"><i class="fa fa-plus fa-lg"></i> Add Version</button>
        </div>
    </div>
    <br/>
<?php
                        ...
                        { 
                            ...
                            <tr>
                                <td align="center" style="white-space:nowrap">
                                    <?php if($_SESSION['LOGINAKS_VISITOR'] == "administrator")
                                    { ?>
                                        <button data-toggle="modal" title="Edit" data-target="#modalEdit" class="btn btn-teal open-edit" data-seq="<?=$row['seq'];?>" data-version="<?=$row['version'];?>" data-date="<?=$row['date'];?>" data-change_log="<?=$row['change_log'];?>" data-req_by="<?=$row['req_by'];?>"><i class="fas fa-edit"></i></button>
                                    
                                        <a href="hapus_versi?seq=<?php echo $row["seq"]; ?>" class="btn btn-danger" title="Delete" onclick="return confirm('Confirm delete version <?php echo " : ".$row["version"]; ?> ?')"><i class="fas fa-trash"></i></a>
                                    <?php } ?>
                                        <button title="Detail" class="btn btn-success" data-toggle="modal" data-target="#modalDetail" id="open-detail" data-version="<?=$row['version'];?>" data-change_log="<?=$row['change_log'];?>"><i class="fas fa-search"></i>
                                    </button>                                    
                                </td>
