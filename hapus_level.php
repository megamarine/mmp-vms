...
$query      = getQuery("select * from m_usermodule where kode_user = '$KODE_USER' and id_module = '6' and xdelete = '1'");
$cek = $query->rowCount();
if($cek == 0)
{
    ?> 
...
else
{
    if(isset($_GET["KODE"]))
    {
        ...
        InsertData(
            ...
            "'', 'Hapus Level - $KODE', '$IP_ADDRESS','$ID_USER1','$DINO','$ID_USER1','Master','Hapus'");

        ?>
        <script>alert('Level has been deleted! Thank you! ');</script>
        <script>document.location.href='m_level.php';</script>
        <?php
        die(0);
    }
}
